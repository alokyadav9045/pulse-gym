        ...
        <div className="flex justify-between items-center h-16">
          <div className="flex-shrink-0">
            <Link href="/" className="flex items-center gap-2">
              <h1 className="text-lg sm:text-xl lg:text-2xl font-bold text-primary">Vitalize Fitness</h1>
            </Link>
          </div>
            ...
            <div className="ml-10 flex items-baseline space-x-1 lg:space-x-2">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  href={item.link}
                  className={`px-2 lg:px-3 py-2 rounded-md text-xs sm:text-sm lg:text-sm font-medium transition-colors ${
